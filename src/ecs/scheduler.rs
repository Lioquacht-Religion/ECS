...
impl SingleThreadScheduler {
    ...
    fn execute_dynamically_planned(schedule: &Vec<SystemId>, systems: &mut Systems, world_data: &mut UnsafeCell<WorldData>) {
        ...
        if let Some(constraint) = systems.get_constraint(&cur_system_id) {
            for next_system_id in constraint.before.iter() {
                if system_id_path.contains(next_system_id) {
                    ...
                    return true;
                } else if find_loops_in_system_constraints_rec(
                    systems,
                    system_id_path,
                    *next_system_id,
                ) {
